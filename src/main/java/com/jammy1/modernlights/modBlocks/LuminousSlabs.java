...
import static com.jammy1.modernlights.util.DefaultBlockSettings.defaultLuminousSlabSettings;

public class LuminousSlabs {

    // Map to store luminous slabs of different colors
    public static final Map<LuminousColors, Block> LUMINOUS_SLABS = new HashMap<>();
    public static final Map<LuminousColors, Block> LUMINOUS_SLABS_FULL = new HashMap<>();
    ...
    private static Block createLuminousSlab(String name, boolean isFull, LuminousColors color) {
        return Util.registerBlocks(name, isFull ? modernLights.fullInfo : null, new LuminousSlabBlock(defaultLuminousSlabSettings(color)));
    }

    // Access Mini Luminous Blocks [just in case]
    public static Block getLuminousSlab(LuminousColors color) {
        return LUMINOUS_SLABS.get(color);
    }

    public static Block getLuminousSlabFull(LuminousColors color) {
